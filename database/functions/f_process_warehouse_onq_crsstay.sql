-- FUNCTION: lookup.f_process_warehouse_onq_crsstay(character varying)

DROP FUNCTION IF EXISTS lookup.f_process_warehouse_onq_crsstay(bigint);
CREATE SCHEMA IF NOT EXISTS lookup;

CREATE OR REPLACE FUNCTION lookup.f_process_warehouse_onq_crsstay(etlBatchId bigint)
    RETURNS integer
    LANGUAGE 'plpgsql'
    COST 100
