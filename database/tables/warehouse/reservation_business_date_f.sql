...
(
    ...
    CONSTRAINT reservation_business_date_f_pk PRIMARY KEY (record_id,business_date)
)
PARTITION BY LIST (business_date);

ALTER TABLE IF EXISTS warehouse.reservation_business_date_f
    OWNER to postgres;

COMMENT ON COLUMN warehouse.reservation_business_date_f.internal_reservation_id IS 'MDO internal reservation id. Each new reservation is given a new internal id. Transfer from staging to reservation_f table creates the a new int_reservation_id';
